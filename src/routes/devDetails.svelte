...
<div class="card bg-info mx-auto mt-3">
  <div class="card-body">
      ...
      <a class="btn btn-sm btn-outline-light mb-2" target="_blank" href="{devobj.portfolioweb}" role="button">Web</a>
      {/if}
      {#if devobj.github!=null}
      <a class="btn btn-sm btn-outline-light mb-2" target="_blank" href="https://github.com/{devobj.github}/" role="button">GitHub</a>
      {/if}      
      {#if devobj.linkedinlink!=null}
      <a class="btn btn-sm btn-outline-light mb-2" target="_blank" href="{devobj.linkedinlink}" role="button">LinkedIn</a>
      {/if}
      {#if devobj.tweetlink!=null}
      <a class="btn btn-sm btn-outline-light mb-2" target="_blank" href="{devobj.tweetlink}" role="button">Twitter</a>
      {/if}
      {#if devobj.fblink!=null}
      <a class="btn btn-sm btn-outline-light mb-2" target="_blank" href="{devobj.fblink}" role="button">Facebook</a>
      {/if}
      {#if devobj.instalink!=null}
      <a class="btn btn-sm btn-outline-light mb-2" target="_blank" href="{devobj.instalink}" role="button">Instagram</a>
      {/if}
      {#if devobj.youtubelink!=null}
      <a class="btn btn-sm btn-outline-light mb-2" target="_blank" href="{devobj.youtubelink}" role="button">Youtube</a>
      {/if}
      </div>
  </div>
</div>
...
<div class="card bg-light my-3 mx-auto">
    ...
    <div class="user text-info mb-2">Skills</div>
        ...
        <span class="bg-info text-light mr-1 px-1">{skill.trim()}</span>
        {/each}
</div>
</div>
...
<div class="row">
    <div class="col-md-6">
        <div class="card border border-light">
            <div class="card-body">
                ...
                <div class="text-dark"><strong>From</strong>: {devexp.frm_date} &nbsp; <strong>To</strong>: {devexp.to_date}</div>
                <div class="text-dark"><strong>Position</strong>: {devexp.job_title}</div>
                <div class="text-dark"><strong>Description</strong>: {devexp.job_des}</div>
                <br>
                {:else}
    ...
    <div class="card border border-light">
            <div class="card-body">
                ...
                <div class="text-dark"><strong>From</strong>: {devedu.frm_date} &nbsp; <strong>To</strong>: {devedu.to_date}</div>
                <div class="text-dark"><strong>Degree</strong>: {devedu.degree}</div>
                <br>
                {:else}
                <div class="text-dark">Not Available</div>
                {/each}
